...
void oc32_print_operand(FILE *file, rtx x, int code)
{
        ...
        case 0:
                ...
                case REG:
                        if (REGNO(operand) > 31)
                                internal_error("unknown register: R%d", REGNO(operand));
                        else
                ...
                case PLUS:
                        output_address(GET_MODE(operand), operand);
                        break;
...
bool oc32_emit_sr_binop(rtx op0, rtx op1)
{
        ...
        /* Step 1: Walk backward through emitted insns looking for AND/OR instruction.
                ...
                For each candidate insn:
                - 1st check insn op0(destination) == WRSR op1(source)
                - 2nd check insn code is AND/OR */
        rtx_insn *insn;
        ...
        if (binop_insn)
        {
                ...
                /* Walk backward from BINOP to find RDSR */
                for (insn = prev_nonnote_insn(binop_insn); insn; insn = prev_nonnote_insn(insn))
                {
                        rtx set = single_set(insn);
                        if (!set)
                                continue;
                        ...
                        if (MEM_P(rdsr_src))
                        {
                                ...
                                {
                                        binop_mask = binop_op1;
                                        break;
                                }
                                else if (rtx_equal_p(rdsr_dest, binop_op1))
                                {
                ...
                if (insn)
                {
                        /* Then: confirm it is RDSR source == WRSR destination
                                Compare MEM_EXPR instead of full MEM to ignore volatile flag */
                        bool match = false;
        ...
        if (rdsr_insn)
        {
                ...
                {
                        /* Search backward from BINOP for instruction defining binop_mask */
                        for (rtx_insn *def_insn = prev_nonnote_insn(binop_insn);
                             def_insn;
                             def_insn = prev_nonnote_insn(def_insn))
                        {
                                rtx set = single_set(def_insn);
                                ...
                                /* dest == binop_mask but mask doesn't match -> FAIL(default) */
                                break;
                        }
                }
        }
...
void oc32_expand_move(machine_mode mode, rtx *operands)
{
        ...
        if (MEM_P(op0) && GET_MODE(op0) == SImode)
        {
                ...
                rtx sr_addr = XEXP(op0, 0);
                HOST_WIDE_INT sr_val = -1;
                if (CONST_INT_P(sr_addr))
