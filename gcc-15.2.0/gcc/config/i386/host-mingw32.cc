...
/* UCRT compatibility: mm_malloc.h uses malloc which is poisoned by GCC.
   We must include windows.h before system.h poisons malloc.  */
  ...
  if (version_info.dwMajorVersion > 4)
    {
      snprintf (local_object_name, sizeof (local_object_name),
		OBJECT_NAME_FMT "%x", (unsigned int)GetCurrentProcessId());
